...
//IMPORTS
import com.example.finalmedieval.exceptions.InvalidCharacterNameException;
...
public class Main extends Application {
    ...
    // Creates Character Name Variable
    String characterName;

    // Creates Variables for Each Image
    static int humanStaffCloth = 0;
    static int humanClothDaggers = 1;
    static int humanClothSword = 2;
    static int humanClothAxe = 3;
    static int humanLeatherStaff = 4;
    static int humanLeatherDaggers = 5;
    static int humanLeatherSword = 6;
    static int humanLeatherAxe = 7;
    static int humanHeavyStaff = 8;
    static int humanHeavyDaggers = 9;
    static int humanHeavySword = 10;
    static int humanHeavyAxe = 11;
    static int elfClothStaff = 12;
    static int elfClothDaggers = 13;
    static int elfClothSword = 14;
    static int elfClothAxe = 15;
    static int elfLeatherStaff = 16;
    static int elfLeatherDaggers = 17;
    static int elfLeatherSword = 18;
    static int elfLeatherAxe = 19;
    static int elfHeavyStaff = 20;
    static int elfHeavyDaggers = 21;
    static int elfHeavySword = 22;
    static int elfHeavyAxe = 23;
    static int orcClothStaff = 24;
    static int orcClothDaggers = 25;
    static int orcClothSword = 26;
    static int orcClothAxe = 27;
    static int orcLeatherStaff = 28;
    static int orcLeatherDaggers = 29;
    static int orcLeatherSword = 30;
    static int orcLeatherAxe = 31;
    static int orcHeavyStaff = 32;
    static int orcHeavyDaggers = 33;
    static int orcHeavySword = 34;
    static int orcHeavyAxe = 35;
    ...
    public void start(Stage primaryStage) {
        ...
        setImageViewProperties(characterImageView);
            // Initializes Stats
        Stats initialStats = new Stats(1, 3, 100); // Default Stats
        updateStatsLabel(initialStats);
        ...
        Button armorTypeSelectionPrevious = new Button("Previous Armor");
        ...
        raceSelectionPrevious.setOnAction(e -> {
            ...
            stats.setAll(defense, damage, health);
        ...
        centerBox.setAlignment(Pos.CENTER); // Aligns everything within the Center Box
        ...
        centerBox.getChildren().add(statsLabel);
        statsLabel.setPrefSize(120, 100);
        ...
        centerBox.getChildren().addAll(finalizeButton);
            // Event Handlers for 'Finalize' Button
        finalizeButton.setOnAction(e -> {
            String forbiddenChars = "[-=!@#$%^&*()_+;',./{}:1234567890]";
            try {
                String name = getString(nameInput, forbiddenChars);

                // If no exception is thrown, the name is valid
                characterName = name;
                System.out.println("Character name successfully finalized: " + characterName);

                // Generate stats string
                String statsString = formatStats(stats);

                // Create a concrete instance of CharacterSummaryScene
                FinalizedScene finalizedScene = new CharacterSummaryScene(characterName, characterImageView, statsString);
                primaryStage.setScene(finalizedScene.createScene(primaryStage));

            } catch (InvalidCharacterNameException ex) {
                nameInput.clear();
                nameInput.setPromptText(ex.getMessage());
                System.err.println("Error Code: " + ex.getErrorCode());
                if (ex.getSuggestion() != null) {
                    System.err.println("Suggestion: " + ex.getSuggestion());
                }
            }
        });
    ...
    @NotNull
    private static String getString(TextField nameInput, String forbiddenChars) throws InvalidCharacterNameException {
        ...
        if (name.isEmpty()) {
            throw new InvalidCharacterNameException(
                    ...
                    1,
                    "Try a name like 'Hero' or 'Warrior'"
            );
        } else if (name.matches(".*" + forbiddenChars + ".*")) {
            throw new InvalidCharacterNameException(
                    "Character name contains forbidden characters or numbers.",
                    2,
                    "Avoid using numbers or special symbols in the name."
            );
        } else if (name.length() < 3 || name.length() > 20) {
            throw new InvalidCharacterNameException(
                    "Character name length must be between 3 and 20 characters.",
                    3,
                    "Use a name with 3 to 20 characters, like 'Knight'."
            );
        }
        return name;
    }

    // Allows for the Stats Label in the GUI to Updates its Numbers
    public void updateStatsLabel(Stats stats) {
        statsLabel.setText(formatStats(stats)); // Update the Stats Label with current stats
    }

    // Formats the Stats Label
    public String formatStats(Stats stats) {
        if(stats.getHealth() == 0.0 && stats.getAttackPower() == 0.0 && stats.getDefense() == 0.0){
            stats.setHealth(100.00);
            stats.setAttackPower(3.0);
       ...
       return String.format("Health: %.2f \nAttack Power: %.2f \nDefense: %.2f", stats.getHealth(), stats.getAttackPower(),
               stats.getDefense());
    }
